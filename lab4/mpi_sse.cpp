...
{
    int j = offset;
    __m128d fvec = _mm_set1_pd(factor);
    for (; j + 1 < len; j += 2)
    {
        __m128d avec = _mm_loadu_pd(a + j);
        __m128d bvec = _mm_loadu_pd(b + j);
        __m128d prod = _mm_mul_pd(bvec, fvec);
        __m128d result = _mm_sub_pd(avec, prod);
        _mm_storeu_pd(a + j, result);
    }
    for (; j < len; ++j)
    {
        a[j] -= factor * b[j];
