...
int main(int argc, char *argv[])
{
    ...
    int n = 0;
    double **A = nullptr, *b = nullptr, *A_1D = nullptr;
    ...
    int *sendcounts = new int[num_procs], *displs = new int[num_procs];
    ...
    int *b_counts = new int[num_procs], *b_displs = new int[num_procs];
    for (int i = 0; i < num_procs; ++i)
    {
    ...
    for (int run = 0; run < num_runs; ++run)
    {
        ...
        for (int k = 0; k < n; k++)
        {
            ...
            if (rank == owner)
            {
                ...
                // cache优化: 预除后直接乘，避免多次除法
                for (int j = k + 1; j < n; ++j)
                {
                    run_local_A[local_k * n + j] *= inv_pivot;
                }
                run_local_A[local_k * n + k] = 1.0;
                run_local_b[local_k] *= inv_pivot;
            ...
            // cache优化: 消元操作，使用局部变量避免cache miss
            for (int i = 0; i < local_rows; ++i)
            {
                ...
                if (global_i > k)
                {
                    ...
                    double *row = run_local_A + i * n;

                    // cache优化: 手动展开 + row_buf提前缓存
                    for (int j = k + 1; j < n; ++j)
                    {
                        row[j] -= factor * row_buf[j];
                    }
        ...
        double *gathered_A = nullptr, *gathered_b = nullptr;
        if (rank == 0)
        {
        ...
        {
            total_time += duration;
            cout << "第 " << run + 1 << " 次用时: " << duration * 1000 << " ms" << endl;
        }
